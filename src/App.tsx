...
const getMovieYear = (movie: Movie): number => {
  ...
  if (movie.releaseDate) {
    const parsed = parseInt(movie.releaseDate.substring(0, 4))
...
function App() {
  ...
  // Fetch movies - get more per page for better client-side filtering
  ...
  const filteredMovies = movies.filter(movie => {
    ...
    if (yearRange.min > 0 || yearRange.max < 9999) {
      ...
      if (movieYear === 0) return true // Include movies without year data
            ...
            <div className="mt-3 p-4 bg-white/5 rounded-xl border border-white/10">
              ...
              {activeFiltersCount > 0 && (
                <p className="mt-3 text-xs text-zinc-600">
                  Showing {displayMovies.length} of {movies.length} movies on this page
                </p>
              )}
            </div>
          )}
